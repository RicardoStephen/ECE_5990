...
def parseFile(fileName):
    with open(fileName) as file:
        ...
        add        = ''
        prevTool   = 19
        #format: ( (x1, y1, z1), (x2, y2, z2), radius, angle)
        arcs       = []
        ...
        for line in file:
            ...
            if(movementType == 'G02' or movementType == 'G2'):
                ...
                distance = length
                arcs.append([[globalX,globalY,globalZ],[prevX,prevY,prevZ],radius,angle])
                #arc_patch(center, radius, 180, 90, ax=ax, fill = 'false',color='green')
            elif(movementType == 'G03' or movementType == 'G3'):
                center, radius, angle, length = drawArc()
                distance = length
                arcs.append(([prevX,prevY,prevZ],[globalX, globalY, globalZ], radius, angle))
        ...
        def Gen_Arc(s, e, r, a):
            length = 50.0
            x_bar = np.divide(s, np.linalg.norm(s))
            n_bar = np.cross(s, e)
            temp = np.cross(n_bar, s)
            y_bar = np.divide(temp, np.linalg.norm(temp))
            t = angle/(length-1)
            arcData = np.empty((3, int(length)))
            for index in xrange(int(length)):
                arcData[:, index] = np.multiply(r, np.add(np.multiply(math.cos(index*t), x_bar), np.multiply(math.sin(index*t), y_bar)))
            return arcData
        ...
        # line_collection = [Gen_Arc(s,e,r,a) for s,e,r,a in arcs]
